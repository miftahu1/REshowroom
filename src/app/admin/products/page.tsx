...
const initialBikeModels: Omit<ProductData, 'id' | 'createdAt'>[] = [
    ...
    },
    {
        ...
        category: 'adventure',
        specs: [
            { value: '40.02', label: 'PS @ 8000 rpm' },
            { value: '40', label: 'Nm @ 5500 rpm' },
            { value: '196', label: 'Kerb Weight' },
        ],
    },
    {
        name: 'Bullet 350',
        engine: '349cc Single-Cylinder',
        price: '₹1.74 L',
        imageUrl: '/assets/images/bullet350.png',
        badge: 'Timeless',
        category: 'classic',
        specs: [
            { value: '20.2', label: 'BHP' },
    ...
    },
    {
        ...
        engine: '349cc Single-Cylinder',
        price: '₹1.50 L',
        imageUrl: '/assets/images/hunter350.png',
        badge: 'Urban',
        category: 'roadster',
        specs: [
            { value: '20.2', label: 'BHP' },
            { value: '27', label: 'Nm Torque' },
            { value: '181', label: 'Kerb Weight' },
        ],
    },
    {
        name: 'Meteor 350',
        engine: '349cc Single-Cylinder',
        price: '₹2.06 L',
        imageUrl: '/assets/images/meteor350.png',
        badge: 'Cruiser',
        category: 'cruiser',
        specs: [
            { value: '20.2', label: 'BHP' },
            { value: '27', label: 'Nm Torque' },
            { value: '191', label: 'Kerb Weight' },
        ],
    },
    {
        ...
        price: '₹3.64 L',
        imageUrl: '/assets/images/meteor650.png',
        badge: 'Super Cruiser',
        category: 'cruiser',
        specs: [
            { value: '47', label: 'PS @ 7250 rpm' },
            { value: '52.3', label: 'Nm @ 5650 rpm' },
            { value: '241', label: 'Kerb Weight' },
        ],
    },
];
...
const ProductModal = ({ isOpen, onClose, product, onSave }: { isOpen: boolean, onClose: () => void, product: ProductData | null, onSave: () => void }) => {
    const getInitialFormState = (): ProductData => ({ name: '', engine: '', price: '', imageUrl: '', badge: '', specs: [], category: 'classic' });
...
const ProductManagement = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if(activeFilter === 'all') {
                ...
                productsQuery = query(collection(db, "products"), where("category", "==", activeFilter), orderBy("createdAt", "desc"));
            }
        ...
        try {
            ...
            initialBikeModels.forEach(model => {
                if (!existingNames.has(model.name)) {
                    ...
                    batch.set(newDocRef, { ...model, createdAt: serverTimestamp() });
                    seededCount++;
                }
            });
            ...
            if (seededCount > 0) {
                await batch.commit();
                alert(`${seededCount} new models have been seeded successfully!`);
                fetchData(); // Refresh data
            } else {
                alert('All models already exist in the database.');
            }
        } catch (error) {
            console.error("Error seeding models: ", error);
            alert('An error occurred while seeding the models.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div>
            <div className="product-management-header" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem'}}>
                <div style={{display: 'flex', gap: '1rem'}}>
                    <button onClick={handleAddClick} className="btn-primary"><i className="fa-solid fa-plus"></i> Add New Model</button>
                    <button onClick={seedModels} className="btn-outline"><i className="fa-solid fa-seedling"></i> Seed Models</button>
                </div>
                <div className="filter-bar" style={{margin: 0}}>
                   <select onChange={(e) => setActiveFilter(e.target.value)} value={activeFilter} style={{padding: '12px 16px', borderRadius: '8px'}}>
                            ...
                            {products.length > 0 ? products.map((p: ProductData) => (
                                ...
                                <tr>
                                    <td colSpan={4} style={{textAlign: 'center', padding: '40px'}}>
                                        <p>No models found for the filter '{activeFilter}'.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
