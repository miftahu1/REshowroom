    ...
    const downloadPdf = () => {
        ...
        if (receiptElement) {
            html2canvas(receiptElement).then(canvas => {
                ...
                pdf.save(`receipt-${receiptData.buyerName.replace(/ /g, '_')}.pdf`);
            });
        }
    };
    ...
    return (
        <div className="admin-page">
            ...
            <div className="grid grid-cols-2 gap-8">
                <div className="form-container glass-card p-6">
                ...
                <div id="receipt-preview" className="p-6 bg-white text-black">
                    ...
                    <div className="flex justify-between mb-4">
                        ...
                        <div>
                            ...
                            <p><strong>Receipt No:</strong> REA-Y{new Date().getFullYear()}M{new Date().getMonth() + 1}-{Date.now().toString().slice(-5)}</p>
                        </div>
                    </div>
                    <hr className="my-4" />
                    <div className="mb-4">
                    ...
                    <table className="w-full mb-4">
                        <thead>
                            <tr><th className="text-left">Description</th><th className="text-right">Amount</th></tr>
                        </thead>
                        <tbody>
                             <tr><td>Ex-Showroom Price ({receiptData.bikeModel})</td><td className="text-right">&#8377; {receiptData.exShowroomPrice.toLocaleString()}</td></tr>
                             <tr><td>RTO Registration</td><td className="text-right">&#8377; {receiptData.rtoPrice.toLocaleString()}</td></tr>
                             <tr><td>Insurance</td><td className="text-right">&#8377; {receiptData.insurancePrice.toLocaleString()}</td></tr>
                             {receiptData.additionalDetails.map((detail, index) => (
                                <tr key={index}><td>{detail.key}</td><td className="text-right">{detail.value}</td></tr>
                            ))}
                        </tbody>
                        <tfoot>
                            <tr><td className="font-bold text-left">Total Amount</td><td className="font-bold text-right">&#8377; {receiptData.totalPrice.toLocaleString()}</td></tr>
                        </tfoot>
                    </table>
                     <div className="mt-6">
            ...
            <div className="flex justify-end mt-8">
                <button onClick={handleSubmit} className="btn-primary">Save & Download Receipt</button>
            </div>
        </div>
    );
};
